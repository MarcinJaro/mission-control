...
const NAV_ITEMS = [
  { label: "Tasks", href: "/", icon: "📋", activeIcon: "📋" },
  { label: "Chat", href: "/chat", icon: "💬", activeIcon: "💬" },
  { label: "Files", href: "/deliverables", icon: "📦", activeIcon: "📦" },
  { label: "Metrics", href: "/metrics", icon: "📊", activeIcon: "📊" },
];
...
export function MobileNav() {
  ...
  return (
    <>
      {/* Mobile Header Bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-zinc-950 z-50 flex items-center justify-between px-4 border-b border-zinc-800 safe-area-top">
        {/* Brand */}
        ...
        <Link
          href="/?new=true"
          className="bg-emerald-600 hover:bg-emerald-500 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-1"
        >
          <span>+</span>
      ...
      {/* Bottom Tab Bar */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-zinc-950 border-t border-zinc-800 z-50 flex items-center justify-around px-2 safe-area-bottom">
        {NAV_ITEMS.map((item) => {
